...
- Static checks pass after these fixes. Version is now 1.0.13.
- Pinned roothide build passed in GitHub Actions run `35511731356` at commit `0bea5cf`: macOS 14 runner, Xcode 15.4 (`15F31d`), Apple clang 15.0.0, iPhoneOS SDK 16.5, no `incompatible arm64e ABI compiler` warning.
- Final roothide artifact: `com.doimty.promotion_1.0.13_iphoneos-arm64e.deb`, SHA256 `7872a0928964eaed12a8ca3b6743580015c894296ba4979ca3b0e4a574499b64`. Package metadata and Mach-O arm64e header verified; required DynamicSource, keepalive, and frame-rate selectors are present.
- Pinned rootless build passed in GitHub Actions run `35512146221` at the same commit, with Xcode 15.4 (`15F31d`), Apple clang 15.0.0, iPhoneOS SDK 16.5, and no `incompatible arm64e ABI compiler` warning.
- Final rootless artifact: `com.doimty.promotion_1.0.13_iphoneos-arm64.deb`, SHA256 `a126a981a550dd08e4845df8d6fbb37d5aa5d9e2c68a7ef22785070f9e508986`.
- Published both artifacts to `https://doimty.github.io/`. Live `Packages` contains exactly the two 1.0.13 architecture stanzas; live `.deb` downloads return HTTP 200 and match the build SHA256 values; `Packages`, `Packages.gz`, `Packages.xz`, and `Packages.lzma` match the live `Release` checksums.
